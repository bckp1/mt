...
// ====== CONFIG ======
const BRAND_NAME = 'MT-VPN STORE';
const STORE_NAME = 'MT-VPN-STORE';
const CONTACT_ADM = '@Sanz_mt';
...
const pADDUDP       = safeRequire('./addudp');
...
function buildMainKeyboard(isOwner) {
  const rows = [
    ...
    [
      { text: '🚀 UDP ZIVPN', callback_data: 'menu:submenu:zivpn' },
...
function buildSubKeyboard(type) {
  const map = {
    ...
    zivpn: [
      { text: '➕ Add Udp ZIVPN', callback_data: 'menu:run:addudp' },
      { text: '🆓 Trial Udp ZIVPN', callback_data: 'menu:run:trialudp' },
    ],
    saldo: [
    ...
    bot.on('callback_query', async q=>{
      try{
        ...
        if(data.startsWith('menu:submenu:')){
          ...
          const titleMap={ssh:'SSH MANAGER',vmess:'VMESS MANAGER',vless:'VLESS MANAGER',trojan:'TROJAN MANAGER',zivpn:'ZIVPN UDP MANAGER',saldo:'TOPUP MANAGER\n\n🔹 Minimal Topup Manual : 1.000\n🔹 Minimal Topup Otomatis : 5000 (Gangguan)',admin:'ADMIN MANAGER'};
        ...
        const map={
          ...
          addssh:pAddSSH, addvmess:pAddVMESS, addvless:pAddVLESS, addtrojan:pAddTROJAN, trialudp:pTRIALUDP,
          topup:pTOPUP, ceksaldo:pSALDO, topupmanual:pTOPMANUAL, datauser:pDATAUSER, addudp:pADDUDP,
