...
// ===== VALIDASI USER UDP =====
function validateUDPUser(username, vps) {
  return new Promise((resolve) => {
    const conn = new Client();
    conn.on('ready', () => {
      conn.exec(`grep -w "^${username}|" /etc/zivpn/users.db`, (err, stream) => {
        if (err) {
          conn.end();
          return resolve({ ok:false, msg:'Gagal membaca database UDP.' });
        }
        ...
        stream.on('close', () => {
          conn.end();
          if (out.trim()) resolve({ ok:true });
          else resolve({ ok:false, msg:`User UDP ${username} tidak ditemukan.` });
        });
      });
    });
...
// ========== PLUGIN UTAMA ==========
function createRenewUDPlugin({ name, aliases=[], title, commandTpl }) {

  global.__renewudp_sessions ??= Object.create(null);
  ...
  async function start(bot, msg) {
    ...
    global.__renewudp_sessions[key] = {
      step: 1, 
      vpsList,
    ...
    setTimeout(() => {
      const S = global.__renewudp_sessions[key];
      if (S && S.step === 1) {
        delete global.__renewudp_sessions[key];
  ...
  async function cont(bot, msg) {
    ...
    if (S.step === 2) {
      if (!/^[A-Za-z0-9_.-]{3,32}$/.test(t)) {
        await send(bot, msg.chat.id, '⚠️ Username ZIVPN tidak valid. 3–32 karakter.');
        return true;
      }
  ...
  async function onCallbackQuery(bot, query) {
    ...
    const S = global.__renewudp_sessions[startKey] || global.__renewudp_sessions[sk];
    if (!S) {
    ...
    if (data === `${name}:cancel`) {
      delete global.__renewudp_sessions[sk];
      delete global.__renewudp_sessions[startKey];
    ...
    await bot.sendMessage(msg.chat.id,
      '👤 Masukkan *username ZIVPN* yang akan diperpanjang:\n\nKlik /batal untuk membatalkan',
      { parse_mode: 'Markdown' }
    );
