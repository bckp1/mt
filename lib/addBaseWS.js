...
async function safeDelete(bot, chatId, msgId) {
  ...
  catch (e) {} // abaikan error (pesan lama / sudah terhapus)
}
...
// simpan message_id
S.prompts ??= [];
S.prompts.push(sent.message_id);
...
if (S.step === 2) {
  // simpan pesan USER (username)
  S.userMsgs ??= [];
  S.userMsgs.push(msg.message_id);
  ...
  const sent = await send(bot, msg.chat.id, '⏳ Masukkan *Masa Aktif (hari)*:');
...
// 🔥 hapus prompt & input lama
const chatId = msg.chat.id;
for (const mid of S.prompts || []) {
  await safeDelete(bot, chatId, mid);
}
for (const mid of S.userMsgs || []) {
  await safeDelete(bot, chatId, mid);
}

delete global.__addws_sessions[key];

// ✅ kirim OUTPUT FINAL SAJA
await send(
  bot,
  msg.chat.id,
  `✅ ${title} berhasil dibuat!\n\n${res.combined || '(output kosong)'}`
);
