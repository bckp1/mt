...
const renewUDP = require('./commands/renewudp');
...
const trialudpPlugin = require('./commands/trialudp');
  ...
  try {
    ...
    if (/^\/batal$/i.test(text)) {
      ...
      return bot.sendMessage(
        msg.chat.id,
        '✅ Semua proses berhasil dibatalkan'
      );
    }
    ...
    for (const n of ['topupmanual', 'trialssh','trialvmess','trialvless','trialtrojan','renewssh',
                     'addssh','addvmess','addvless','addtrojan','renewvless','addudp','renewudp',
...
const addudp = commandMap.get('addudp');
if (addudp?.onCallback) {
  const handled = await addudp.onCallback(bot, query);
  if (handled) {
    await bot.answerCallbackQuery(query.id).catch(()=>{});
    return;
  }
...
if (commandMap.get('riwayat')?.onCallback) {
  ...
  if (handled) {
    ...
    if (data.startsWith('renewudp:')) {
      await renewUDP.onCallbackQuery(bot, query);
      return;
    }
